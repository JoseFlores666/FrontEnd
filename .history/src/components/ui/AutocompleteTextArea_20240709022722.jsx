...
export const AutocompleteTextArea = ({
    ...
    const handleSelectWord = (selectedWord) => {
        ...
        if (!recentSuggestions.includes(selectedWord)) {
            const newSuggestions = recentSuggestions.length >= 5 ?
                [...recentSuggestions.slice(1), selectedWord] :
                [...recentSuggestions, selectedWord];

            setRecentSuggestions(newSuggestions);
        }
    ...
    return (
        <div ref={containerRef}>
            <input
                className="p-3 text-black w-full rounded-md border border-gray-400 focus:outline-none focus:border-blue-500"
