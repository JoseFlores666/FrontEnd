...
export const AutocompleteTextArea = ({
    ...
    // Ruta del campo a extraer
    const descripcionPath = 'informe.descripcionDelServicio';
    ...
    const extractWords = (data, fieldPath) => {
        if (!data) {
            return [];
        }
        ...
        const allWords = fieldValue.toLowerCase().split(' ');
    ...
    const handleValueChange = (value) => {
        ...
        if (value.trim() === "") {
            const suggestions = extractWords(data, descripcionPath).slice(0, 5);
            setFilteredWords(suggestions);
            setShowHistorial(null);
        } else {
            const words = value.toLowerCase().split(' ');
            const lastWord = words[words.length - 1];
            const suggestions = extractWords(data, descripcionPath)
