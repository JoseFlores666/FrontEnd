...
export const AutocompleteInput = ({
  ...
  useEffect(() => {
    // Listener para el evento personalizado 'keepFocus'
    ...
    descripcionRefs.current[index]?.addEventListener('keepFocus', handleKeepFocus);

    return () => {
      descripcionRefs.current[index]?.removeEventListener('keepFocus', handleKeepFocus);
    };
  }, [index]);
  ...
  const handleSelectWord = (selectedWord) => {
    ...
    // Mantener el foco en el mismo campo de entrada
    if (descripcionRefs.current[index]) {
      // Usar un evento personalizado para mantener el foco después de actualizar el valor
      const keepFocus = new Event('keepFocus');
      descripcionRefs.current[index].dispatchEvent(keepFocus);
    }
  };
