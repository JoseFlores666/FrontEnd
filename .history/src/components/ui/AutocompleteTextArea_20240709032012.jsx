...
export const AutocompleteTextArea = ({
    ...
    const handleValueChange = (value) => {
        ...
        const extractWords = (data, field) => {
            const allWords = data.flatMap(item =>
                item[field].map(ins => ins.descripcion.toLowerCase().split(' ')).flat()
            );
            return [...new Set(allWords)];
        };
    
        if (value.trim() === "") {
            const suggestions = extractWords(data, 'solicitud.insumosSolicitados').slice(0, 5);
            setFilteredWords(suggestions);
            setShowHistorial(null);
        } else {
            const words = value.toLowerCase().split(' ');
            const lastWord = words[words.length - 1];
            const suggestions = extractWords(data, 'solicitud.insumosSolicitados')
