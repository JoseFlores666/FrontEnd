...
export const AutocompleteInput = ({
  ...
  tipo = false // Bandera para alternar entre input y textarea
}) => {
  ...
  const handleSelectWord = (selectedWord) => {
    ...
    // Disparar un evento personalizado para mantener el enfoque
    const keepFocusEvent = new Event('keepFocus');
    inputRefs.current[index].dispatchEvent(keepFocusEvent);
  };
  ...
  const InputComponent = tipo ? 'input' : 'textarea';

  return (
    <div ref={containerRef} className="relative">
      <InputComponent
        className={`p-3 text-black w-full rounded-md resize-none border border-gray-400 ${tipo ? 'text-sm' : ''}`}
      ...
      {showHistorial && (
        <div
          ...
          style={{ width: inputRefs.current[index]?.offsetWidth }}
        >
