...
export const RegisterSolicitudPage = () => {
  ...
  const { crearmySoli, getIdsProyect, ids, getIdsProyectYAct, idsAct = [], unasoli, traeFolioInterno, myFolioInterno, actializarSoli, getunSolitud,
    getFirmas,
    nombresFirmas,
  } = useSoli();
  ...
  useEffect(() => {
    if (editar && unasoli || duplicar && unasoli) {
      ...
      if (unasoli.proyecto) {
        ...
        // getIdsProyectYAct(unasoli.proyecto);//hace la consulta de las actividades enlzadas a ese proyecto
        setTimeout(() => {
          if (unasoli.actividades) {
            if (unasoli.actividades && unasoli.actividades.length > 0) {
              // Por ahora, asumo que quieres manejar la primera actividad en caso de duplicar o editar
              const primeraActividad = unasoli.actividades[0]; // Puedes cambiar la lógica según tus necesidades
