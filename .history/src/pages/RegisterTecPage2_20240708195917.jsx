...
export const RegisterTecPage2 = () => {
    ...
    const handleClickOutside = (event) => {
        if (
            descripcionRefs.current.every((ref, index) => ref && !ref.contains(event.target)) &&
            !event.target.classList.contains("historial-item") // Añadir una clase al span para identificarlo
        ) {
    ...
    useEffect(() => {
        console.log("showHistorial:", showHistorial);
    ...
    const toggleHistorial = (index) => {
        console.log("Toggling historial for index:", index);
    ...
    const handleSelectHistorial = (index, selectedText) => {
        console.log(`Seleccionado historial para el ítem ${index}: ${selectedText}`);
                            ...
                            <div key={index} className="grid grid-cols-12 gap-6 mb-4">
                                ...
                                <div className="col-span-6">
                                    ...
                                    <textarea
                                        ...
                                        onClick={() => toggleHistorial(index)} // Verifica que toggleHistorial se llame correctamente
                                        readOnly // Para evitar ediciones directas en el textarea
                                    />
                                    ...
                                    {showHistorial === index && (
                                        <div
                                            ...
                                            onClick={(e) => e.stopPropagation()} // Evitar propagación de clics dentro del historial
                                        >
                                            {historialOrden.map((histItem, histIndex) => (
                                                <div key={histIndex} className="mb-2">
                                                    <span
                                                        className="text-sm cursor-pointer historial-item"
                                                        onClick={(e) => {
                                                            e.preventDefault(); // Evitar comportamiento predeterminado (si hay alguno)
