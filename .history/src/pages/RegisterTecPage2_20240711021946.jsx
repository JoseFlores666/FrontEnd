...
const RegisterTecPage2 = () => {
    ...
    const onSubmit = async (data, e) => {
        ...
        try {
            ...
            const parsedDate = parse(fechaOrden, 'yyyy/MMMM/iii', new Date());
                    ...
                    <div className="p-4 space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    className="flex h-10 w-2/5 text-indent:4 rounded-md  border-input px-3 py-2 border-gray-400  bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 "
                                    id="fechaOrden"
                                    type="date"
                                    value={fechaOrden}
                            ...
                            <div className="flex flex-col items-end">
                                <label className="text-sm font-medium leading-none text-right mr-18" htmlFor="folioExterno">
                                    No. de folio Externo:
                                </label>
                                <div className="flex items-center">
                                    <input
                                        className="h-10 w-3/4 rounded-md border border-input px-3 py-2 border-gray-400 bg-gray-50 text-gray-900 focus:ring-blue-500 focus:border-blue-500"
                                        id="folioExterno"
                                        type="number"
                        ...
                        <div className="border rounded-lg bg-white">
                                ...
                                <table className="w-full caption-bottom text-sm border">
                                    <thead className="[&_tr]:border border-gray-400">
                                        <tr className="border transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted border-gray-400">
                                            <th className="h-12 text-center px-4  align-middle font-medium text-black [&:has([role=checkbox])]:pr-0 border-gray-400">
                                                Cantidad
                                            </th>
                                            <th className="h-12 px-4 text-center align-middle font-medium text-black [&:has([role=checkbox])]:pr-0 border-gray-400">
                                                Descripción
                                            </th>
                                            <th className="h-12 px-4 align-middle  font-medium text-black [&:has([role=checkbox])]:pr-0 border-gray-400">
                                                Acción
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody className="[&_tr:last-child]:border-0  border-gray-400">
                                        {items.map((item, index) => (
                                            <tr key={index} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted  border-gray-400">
                                                <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 border border-gray-400">
                                                    <div className="flex  justify-center">
                                                        <input
                                                            className="flex h-10 w-3/   text-center rounded-md border border-input  px-3 py-2 border-gray-400 bg-gray-50  text-gray-900 focus:ring-blue-500 focus:border-blue-500 "
                                                            type="number"
                                                            value={item.cantidad}
                                                ...
                                                <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 border border-gray-400">
                                                    <AutocompleteInput
                                                        ...
                                                        inputProps={{
                                                            type: "text",
                                                            maxLength: 200,
                                                            className: "w-full text-black p-3 border border-gray-400 bg-gray-50 rounded-md focus:ring-indigo-500 focus:border-indigo-500",

                                                        }}
                                                    />
                                                </td>
                                                <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 text-right border border-gray-400">
                                                    <button
                            ...
                            <div className="p-4 bg-white border-b  border-r border-l border-gray-400 rounded-b-md">
                                <button
                                    onClick={agregarItem}
                                    class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background focus-visible:ring-ring border-input bg-background hover:bg-accent hover:text-accent-foreground   border-gray-200 ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-white hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 w-full"
                                >
                                    Agregar más
                                </button>
                            </div>
