...
export const RegisterTecPage2 = () => {
    ...
    const handleDescripcionChange = (index, value) => {
        ...
        if (lastWord.trim() !== "" && value.endsWith(" ")) {
            ...
            setShowHistorial(index); // Mostrar sugerencias cuando se deje un espacio
        } else {
            setFilteredWords([]);
            setShowHistorial(null); // Ocultar sugerencias si no hay valor
        }
    };
    ...
    const handleKeyDown = (index, event) => {
        if (event.key === "Tab") {
            if (filteredWords.length > 0) {
                ...
                handleSelectWord(index, filteredWords[0]);
            } else {
                const newItems = [...items];
                const suggestedWord = newItems[index].descripcion ? filteredWords[0] : "";
                newItems[index].descripcion += suggestedWord;
                setItems(newItems);
                setShowHistorial(null); // Ocultar sugerencias si no hay palabras filtradas
            }
        }
    };
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="bg-white p-6 rounded-md shadow-md">
                        ...
                        {items.map((item, index) => (
                            <div key={index} className="grid grid-cols-12 gap-6 mb-4">
                                ...
                                <div className="col-span-6">
                                    ...
                                    {showHistorial === index && filteredWords.length > 0 && (
                                        <div
                    ...
                    <div className="flex justify-center mt-4">
                        <button
                            type="submit"
                            className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-3 px-6 col-span-3 rounded-md border border-black ${isValid ? "" : "opacity-50 cursor-not-allowed"}`}
                            disabled={!isValid}
                        >
                            Guardar
                        </button>
