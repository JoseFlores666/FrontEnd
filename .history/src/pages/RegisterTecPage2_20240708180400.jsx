...
export const RegisterTecPage2 = () => {
    ...
    const [historialSeleccionado, setHistorialSeleccionado] = useState(""); // Estado para almacenar el historial seleccionado
    ...
    const handleSelectHistorial = (index, selectedText) => {
        ...
        newItems[index].descripcion = selectedText; // Establecer el texto seleccionado como la descripción

        // Filtrar el historial correspondiente al seleccionado
        const filteredHistorial = historialOrden.filter(item => item.informe.descripcionDelServicio === selectedText);
        newItems[index].historial = filteredHistorial.map(item => item.informe.descripcionDelServicio);

        setItems(newItems);
        setHistorialSeleccionado(selectedText); // Almacenar el historial seleccionado
        setShowHistorial(null); // Cierra el historial seleccionado
    };
                                                ...
                                                item.historial.map((hist, histIndex) => (
                                                    <div key={histIndex} className="mb-2">
                                                        <span
                                                            ...
                                                            onClick={() => handleSelectHistorial(index, hist)}
                                                        >
                                                            {hist}
                                                        </span>
                                                    </div>
