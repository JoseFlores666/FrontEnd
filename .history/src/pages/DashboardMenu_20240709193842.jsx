...
import { faKey, faEye, faEyeSlash, faSyncAlt, faUserPlus, faRedo } from '@fortawesome/free-solid-svg-icons';
...
export const DashboardMenu = ({ isOpen, toggleMenu }) => {
    ...
    const [isReloading, setIsReloading] = useState(false); // Estado para manejar la animación de recarga

    const { traeApis_keys, api_Key, actualizaApiKey } = useSoli();
    ...
    useEffect(() => {
        const llamaApi = async () => {
            try {
                console.log("Llamando a traeApis_keys...");
                await traeApis_keys();
                console.log("Datos recibidos de traeApis_keys:", api_Key);
                setEsperarApiKeys(true);
                setDatosCargados(true);
                setIsReloading(false); // Detenemos la animación de recarga
            } catch (error) {
                console.error("Error al consultar las API keys:", error);
                Swal.fire("Error al guardar los datos", "", "error");
                setIsReloading(false); // Detenemos la animación de recarga en caso de error
            }
        };
        if (!esperarApiKeys) {
            llamaApi();
    ...
    const llenaSolicitud = () => {
        try {
            ...
            console.log("Llenando el input con api_Key:", key);
            setValue("apiKey", key);
        } catch (error) {
    ...
    const handleUpdateApiKey = async (data) => {
        try {
            await actualizaApiKey(data.newApiKey);
            Swal.fire("API Key actualizada correctamente", "", "success");
            setValue("apiKey", data.newApiKey);
            handleCloseModal();
        } catch (error) {
    ...
    const handleReloadApiKeys = async () => {
        setIsReloading(true); // Iniciamos la animación de recarga
        ...
        isOpen && (
            <div ref={menuRef} className="absolute w-full right-0 mt-5 bg-white text-black rounded-md shadow-lg z-10 flex items-center justify-between p-4">
                ...
                <button
                    ...
                    className={`ml-2 p-2 border border-gray-300 rounded bg-gray-200 hover:bg-gray-300 ${isReloading ? 'reloading' : ''}`} // Aplicamos la clase 'reloading' cuando está recargando
                >
