        ...
        <div className="mx-auto max-w-6xl p-4 text-black">
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="bg-white p-6 rounded-md shadow-md">
                    ...
                    {justificaciones.map((value, index) => (
                        <AutocompleteInput
                            key={index}
                            index={index}
                            value={value}
                            onChange={(newValue) => handleJustificacionChange(index, newValue)}
                            data={[]} // Aquí debes pasar tu arreglo de datos para autocompletado
                            recentSuggestions={[]} // Aquí puedes manejar las sugerencias recientes si lo necesitas
                            setRecentSuggestions={() => { }} // Puedes pasar una función vacía si no se usan sugerencias recientes
                            descripcionRefs={[]} // Aquí puedes manejar las referencias a los elementos de justificación
                            fieldsToCheck={["campo1", "campo2"]} // Campos a considerar para autocompletado
                        />
                    ))}
